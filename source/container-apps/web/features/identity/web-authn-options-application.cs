...
// X-Forwarded-Host is never consulted. HttpRequestHostAccessor prefers X-TimeWarp-Circuit-Host
// when IdentitySessionCookieForwardingHandler set it on InteractiveServer HTTPS loopback, else
// Request.Host. That loopback does not rewrite HTTP Host (TLS must keep validating localhost).
// The circuit host is copied from the circuit request, not from a client-supplied forwarded header.

#region Purpose
// Scheme-agnostic ICurrentPrincipalAccessor implementation: reads the principal-id claim off
// whichever scheme's ClaimsPrincipal the authorization middleware attached to HttpContext.User.
#endregion
...
// (unlike AgentCallerContext) — see ICurrentPrincipalAccessor's Design region for why a policy
// accepting EITHER scheme (credential-management) makes both of those unnecessary: by the time a
// handler behind that policy runs, HttpContext.User already IS the winning scheme's ClaimsPrincipal.
// Reading the claim directly off HttpContext.User is therefore both simpler and correct for any
// scheme the policy admits, including future schemes added to CredentialManagementDefaults.Policy's
// AddAuthenticationSchemes list without this class needing a corresponding change.

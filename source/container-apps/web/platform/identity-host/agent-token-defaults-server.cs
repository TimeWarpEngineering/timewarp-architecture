...
// IdentityReadPolicy and DemoInvokePolicy restrict AuthenticationSchemes to ONLY this scheme — a
// request carrying nothing but a valid identity-session cookie is never even offered to this
// scheme's handler, which is what makes scheme isolation structural rather than incidental (see
// AgentTokenAuthenticationHandler's Design region and Agent_Protected_Endpoint_Tests's
// cookie-session-cannot-reach-bearer-endpoint case). DemoInvokePolicy gates the metered
// pay-for-capability demo (104-011). ScopeClaimType is a DIFFERENT claim type than
// IdentitySessionDefaults.PrincipalIdClaimType (which IS reused here for the principal-id claim —
// same identity concept, same claim type, deliberately shared) — a ClaimsPrincipal from either
// scheme therefore carries the principal id under one consistent claim type, while scope claims
// (agent-token-specific, no cookie-scheme equivalent) get their own type.
#endregion
...
public static class AgentTokenDefaults
{
  ...
  /// <summary>Bearer policy requiring <see cref="AgentScopes.DemoInvoke"/> (metered demo, 104-011).</summary>

...
// the precise merged-identity claim-resolution model: a dual-scheme route (credential.manage.self
// with AuthenticationSchemes identity-session + agent-token on the contract) makes both of those
// single-scheme techniques unnecessary because HttpContext.User, by the time a handler runs,
// already carries a ClaimsIdentity for EVERY scheme the request successfully authenticated against
// (ordinarily exactly one), not a single "winning" scheme's principal. Reading the claim directly
// off HttpContext.User is therefore both simpler and correct for any scheme the endpoint lists,
// including future schemes added to [EndpointAuthorize(AuthenticationSchemes)] without this class
// needing a corresponding change; in the unusual both-succeeded case, FindFirstValue's merge-order-dependent

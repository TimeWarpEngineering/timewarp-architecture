...
|----|-----|--------|---------|-------------|
| G1 | minor | fixed | standalone yarp `RequestHeaderOriginalHost` on WebRoute/WebSwaggerRoute doesn't reach passkeys: `/api/identity/*` matches the pre-existing `ApiRoute` catch-all → Api.Server. Fuller review: this is a PRE-EXISTING gap (standalone yarp never routed identity to Web.Server), NOT a 104-031 regression; the transform added is harmless; the AppHost path (task-112, verified) is correct. | FIX (minimal, no scope creep): keep the WebRoute/WebSwaggerRoute transforms (correct for those routes); correct only the false "standalone yarp covers passkey host" claim; note the pre-existing identity-routing gap relates to task 107 (yarp route generation), not fixed here. |
| S1 | minor | documented | AllowedRpIds append-only → `localhost` can never be removed in prod; empty-AllowedOrigins fallback permanently accepts rp.id=localhost (confined to loopback self-registration, not remotely exploitable) | DOC: options Design region — accepted for the template default; note the loopback-confined posture. Steve-flagged. |
| S2 | minor | documented | anonymous Start endpoints are an allowlist-membership oracle (200 vs 400) | DOC: scope the "no enumeration" claim to "no host reflected"; membership of public hostnames is low-value |
| S3 | minor | documented | flat AllowedOrigins across all RP IDs = footgun if populated in multi-RP prod (blocked one layer down by browser rpId-suffix + rpIdHash) | DOC: already noted in options Design region as out-of-scope; confirm wording covers the multi-RP risk |
| G2i | nit | accepted | ingress host-preservation has no automated coverage (host-selection tests hit web-server directly); rests on manual live-chain check | ACCEPT: record as known gap in Results |
| G2 | nit | fixed | Passkey_HostSelection_Tests cert-override comment imprecise | RESOLVED (empirical, build agent): the override is REQUIRED, not redundant — removing it fails 3 non-localhost cases with AuthenticationException. In SocketsHttpHandler, `Headers.Host` DOES move the server-CERTIFICATE-NAME validation target (derived from request authority), so Host="webauthn-second.test" name-mismatches the localhost dev cert and aborts the handshake; TCP still targets localhost:7000. Comment corrected to state this precisely; override kept. My earlier 'redundant' framing (and the false 'Headers.Host doesn't change SNI' claim) was WRONG. |
| S4 | nit | accepted | AllowedHosts:"*" leaves per-request allowlist as sole host gate | ACCEPT: acceptable; optional defense-in-depth |
| S5 | nit | accepted | hermetic strip exact-matches Path=="secrets.json" (framework-fragile) | ACCEPT: pinned by binding test — fails loud if framework changes |

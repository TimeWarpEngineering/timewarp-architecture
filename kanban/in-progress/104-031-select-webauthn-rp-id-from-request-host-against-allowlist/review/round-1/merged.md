...
| G2 | nit | fixed | Passkey_HostSelection_Tests SNI/dev-cert comment overstates — Headers.Host doesn't change TLS SNI (URI authority does); DangerousAcceptAnyServerCertificateValidator likely redundant | FIX: drop the redundant cert override or correct the comment (test-only; outcome unaffected) |

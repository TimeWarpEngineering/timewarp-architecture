...
- [x] Inventory: confirm the 26 marker files + any `typeof(*AssemblyMarker)` consumer sites;
      verify RootNamespace ↔ declared-namespace agreement per project (fix mismatches first)
- [x] Add the generation target to root `Directory.Build.targets` (create if absent; confirm
      it ships as template content so generated apps inherit it); incremental-build-safe
- [x] Convert `internals-visible-to-client-and-server.cs` to `<InternalsVisibleTo>` items on
      web-contracts.csproj; delete the file
- [x] Delete all 26 `assembly-marker.cs` files; normalize web-spa consumers to `IAssemblyMarker`
- [x] Verify TWA0004 behavior on the generated file (skip or embedded Purpose region)
- [x] Update AGENTS.md AssemblyMarker line + any skill/doc that instructs creating the file
      manually (present tense, no history in public skills)
- [x] Gates: `dev build` 0/0 (full), `dev test`, `dev template-smoke` both matrices via
...
MSBuild-generated `IAssemblyMarker` for all former product/platform marker assemblies; deleted 26 checked-in `assembly-marker.cs` files; converted web-contracts IVT to SDK items with verified AssemblyNames; normalized SPA (including Routes.razor) to `IAssemblyMarker`; packed root `Directory.Build.targets` in the template; updated AGENTS.md and ADR-0002.

### Files changed
| Path | Action |
|------|--------|
| `Directory.Build.targets` | Created — GenerateAssemblyMarker + IncludeGeneratedAssemblyMarker |
| `source/container-apps/Directory.Build.props` | AssemblyMarkerNamespace maps + aspire opt-out |
| `source/foundation/Directory.Build.props` | Foundation Common.* maps |
| `source/analyzers/Directory.Build.props` | Analyzers/Attributes maps; convention-analyzers opt-out |
| `source/libraries/Directory.Build.props` | Created — Identity/Modules maps |
| `web-contracts.csproj` | SDK InternalsVisibleTo |
| SPA program / Routes / web-server program / AspireSpaTestApplication | IAssemblyMarker |
| 26× `assembly-marker.cs` + IVT .cs | Deleted |
| template csproj | Pack Directory.Build.targets |
| AGENTS.md, ADR-0002 | Present tense |

### Key decisions / deviations
1. **`AssemblyMarkerNamespace`** required — RootNamespace alone is wrong for container-apps (shared) and foundation/libraries (kebab defaults).
2. IVT friend names corrected to real AssemblyNames (`web-server`, not `Web.Server`).
3. MSBuild `;` escaped as `%3b` in generated source lines.
4. Split write vs include targets for incremental-safe Compile.
5. Routes.razor was a missed SPA consumer (bound to TimeWarp.State.AssemblyMarker) — fixed in review round 1.
...
| `dev build` | 0/0 |
| `dev test` | All passed |
| `dev template-smoke` | Both matrices OK; SmokeDefault.Web.Server/Spa markers generated; 0 checked-in markers in output |

### Review (Phase 4b)
- **Rounds:** 2
- **Effort / roster:** 1 — general
- **Final counts:** 0 open; 1 bug fixed; 1 suggestion fixed
- **Disposition:** clean

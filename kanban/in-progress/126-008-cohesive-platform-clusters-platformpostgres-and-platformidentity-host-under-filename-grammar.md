...
- [ ] Resolve glob coverage for `platform/` in the g.props generator + membership guard
      (STOP if non-trivial — design issue, not workaround)
- [ ] `git mv` + grammar-rename the 12 files per the tables; delete emptied subfolders
- [ ] Update the five postgres paths in `.template.config/template.json` (!postgres) excludes
- [ ] Reconcile Purpose/Design regions narrating old homes
- [ ] Sweep for stale path references (docs, tests, workflows, skills)
- [ ] Update AGENTS.md Layout (add `platform/` line) + `skills/tw-feature-placement/SKILL.md`
      (platform tree: same grammar, NOT slice-namespaced, when to use platform/ vs features/ vs
      host — present tense, no history)
- [ ] Gates: `dev build` 0/0 (FULL rebuild — glob/generator surface changed), `dev test`,
...
- [ ] Runtime spot check: postgres profile boots (aspire or web-server with connection) —
